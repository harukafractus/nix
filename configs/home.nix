  ...
  programs = {
    ...
    zsh = {
      ...
      initContent = ''
        ...
        ignore_patterns=(
          "ls" "ls *" "la" "ll"                     # Standard Listings
          "exa *" "eza *" "tree" "tree *"           # Enhanced Listings
          "cd" "cd *" "pwd" "less"                  # Standard Navigation
          "popd" "popd *" "pushd" "pushd *" "dirs"  # Stack Navigation
          "clear" "exit" "history" "date"           # Session/Time
          "jobs" "fg" "bg"                          # Job Control
          "man *" "which *" "file *"                # Help/Lookup
          "open *" "codium *"                       # Opening files
          "ping *" "dig *" "nslookup *"             # Network Diagnostics
          
          # --- Environment Activation Noise ---
          "source .venv*" "source venv*"            # Ignore standard venv activation
          "conda activate *"                        # Ignore conda activation
          
          # --- Git Ignore List ---
          "git status" "git status *"               # Check state
          "git add *"                               # Staging (routine)
          "git diff *"                              # Reviewing (read-only)
          "git log *" "git show *"                  # Reading history
          "git branch *"                            # Listing/creating branches
          "git switch *" "git checkout *"           # Changing branches (navigation)
          "git fetch *" "git pull *" "git push *"   # Syncing (routine)
          "git stash *" "git restore *"
          "git commit *" "git rebase *"             # YES I KNOW WHAT IM DOING
        )
        
        # Join the array with '|' and wrap in parenthesis
        HISTORY_IGNORE="(''${(j:|:)ignore_patterns})"
        ...
        zshaddhistory() {
          ...
          #  Echo/Cat (ignore unless piping/redirecting)
          if [[ "$cmd" == (echo|cat|less) ]]; then
            if [[ "$line" != *['|><']* ]]; then
              return 1
            fi
          fi
